package com.sample.core.business.usecase

import com.google.gson.Gson
import com.sample.core.business.entity.*
...
abstract class MainUseCase<T : List<BaseResponse>, Params> {
    ...
    suspend fun execute(
        ...
        try {
            ...
            if (response.isSuccessful) {
                ...
                body?.let {
                    if (it.isEmpty())
                        return ItemResult.Error("List is empty")
                    return ItemResult.Success(it)
                }
                return ItemResult.Error(" ${handleNetworkRequestError(HttpException(response))}")
            } else {
                return ItemResult.Error(" ${handleNetworkRequestError(HttpException(response))}")
            }
        } catch (e: Exception) {
